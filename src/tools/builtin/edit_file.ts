  ...
  async execute(params: Record<string, unknown>, ctx: ToolContext): Promise<ToolResult> {
    try {
      ...
      if (!content.includes(oldString)) {
        return { success: false, output: '', error: 'oldString not found in file content' };
      }

      const count = content.split(oldString).length - 1;
      if (count > 1) {
        return { success: false, output: '', error: `Found ${count} matches for oldString. Provide more context to make it unique.` };
      }

      const updated = content.replace(oldString, newString);
      const bytes = await Bun.write(filePath, updated);

      return { success: true, output: `Edited ${filePath} (${bytes} bytes)`, metadata: { path: filePath, bytes } };
    } catch (e) {
