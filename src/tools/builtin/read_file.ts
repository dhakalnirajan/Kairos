...
export const readFileTool: ToolInstance = {
  ...
  async execute(params: Record<string, unknown>, ctx: ToolContext): Promise<ToolResult> {
    try {
      const filePath = resolveFilePath(String(params['path'] ?? params['file'] ?? params['param'] ?? ''), ctx.workspaceRoot);
      ...
      return { success: true, output: sliced, metadata: { path: filePath, totalLines: lines.length, returnedLines: Math.min(limit, lines.length - offset) } };
    } catch (e) {
